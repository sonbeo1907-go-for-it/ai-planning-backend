...
The MVP supports self-registered internal email/password accounts and Google
identities. Authentication uses short-lived access JWTs and database-backed,
rotated refresh tokens. Both login methods issue the same application session
pair. Self-service password recovery remains outside the current baseline.
