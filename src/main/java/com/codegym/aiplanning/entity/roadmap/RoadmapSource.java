...
@Table(
        ...
        uniqueConstraints = {
            ...
            @UniqueConstraint(
                    name = "uk_roadmap_sources_roadmap_material",
                    columnNames = {"roadmap_id", "material_id"})
        })
public class RoadmapSource extends BaseEntity {
    ...
    @JoinColumn(name = "learning_source_id")
    ...
    public static RoadmapSource link(Roadmap roadmap, Material material) {
        if (!roadmap.getOwner().getId().equals(material.getUser().getId())) {
            throw new IllegalArgumentException("Roadmap and Material owners must match.");
        }
        RoadmapSource link = new RoadmapSource();
        link.roadmap = roadmap;
